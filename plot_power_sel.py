...
if __name__ == "__main__":
    for n in [5, 10, 15, 20]:#5, 10, 15, 20
        for test_mode in ["betabinom.comb"]:#"mean", "chi2","kw", "beta", "betabinom.comb"
            confs = []
            for sel_mode in ["ts.5", "ts", "equal", "beta.med", "means", "mean.slow"]:# "ts.5", "ts", "equal", "beta.med", "means", "mean.slow", "beta"
            ...
            plot_power(confs, title_func=title, label_func=label, save_func=save, post_func=post, load_path="./test_res/H1", save_path = "./power_plots/sel.mode", line_colors=colors)