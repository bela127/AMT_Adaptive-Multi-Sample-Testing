            ...
            for test_mode in ["betabinom.comb"]:
                for m in [0, 1]: # m=0 means H0, m=1 means H1 where m=1 internally sets m to the number of samples N contained in the dataset
                    if m == 0:
                        ...
                        exp = Experiment(conf=conf, load_path=f"./coin_res/real_h0/{dataset}", save_path=f"./multi_stat_res/real_h0/{dataset}")
                        ...
                        exp = Experiment(conf=conf, load_path=f"./coin_res/real/{dataset}", save_path=f"./multi_stat_res/real/{dataset}")
                    exp.run_parallel()
                    exp.save()
                    del exp
                    gc.collect()