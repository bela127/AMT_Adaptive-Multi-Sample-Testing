...
load_path = "./coin_res"
save_path = f"./coin_plot/diff_single"
conf = Config(
                n = 5,
                m = 1,
                sample_size = 2000,
...
stat_name = conf.get_sel_name()
stat_contingency = np.load(f"{load_path}/H1/contingency_{stat_name}.npy")[:,:,0,:]

null_conf = conf.clone()
null_conf.m = 0
null_name = null_conf.get_sel_name()
null_contingency = np.load(f"{load_path}/H0/contingency_{null_name}.npy")[:,:,0,:]

makedirs(save_path, exist_ok=True)

stat_sample_size = np.sum(stat_contingency, axis=1)
null_sample_size = np.sum(null_contingency, axis=1)

stat_values = stat_contingency[:, 0] / stat_sample_size
null_values = null_contingency[:, 0] / null_sample_size
...
for i in sizes:
    ...
    plt.legend()
    plt.savefig(f"{save_path}/dist_diff_sample_size{i}__{stat_name}.png")
    plt.show()