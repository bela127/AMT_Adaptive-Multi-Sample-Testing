...
if __name__ == "__main__":
    ...
    for n in [20]:#
        for sel_mode in ["ts", "equal", "beta", "mean.slow"]:# "ts.5", "ts", "equal", "beta", "means", "mean.slow"
            ...
            max_sel_average = np.mean(max_sel, axis = 0)
            sel_average = np.mean(coin_percent, axis = 0)

            pl = plt.plot(sizes,sel_average[19], label=f"{conf.selection_mode}, plated")
            c = pl[-1].get_color()
            plt.plot(sizes, max_sel_average, label=f"{conf.selection_mode}, max", color=c, linestyle="--")
