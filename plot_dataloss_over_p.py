...
if __name__ == "__main__":
    ...
    n, sel_mode, m, max_iterations, p_diff = 20, "beta.med", 1, 2000, 0.1
    ...
    for val in common_p_values:
        dummy_conf = Config(
            n=n, m=m, sample_size=max_iterations, initial_size=INITIAL_SIZE_PER_ARM,
            common_p=val, p_diff=p_diff, 
...
# ---------------------------------------------------------------
    ...
    valid_x = [val for val in common_p_values if val in our_power_curves]
    ...
    for test_mode in test_modes:
        ...
        for val in common_p_values:
            ...
            # We always record the x-location so the x-axis alignment remains continuous
            x_points.append(val)
        ...
        # Plot if at least one non-NaN point exists
        if len(y_ratio_points) > 0 and not np.all(np.isnan(y_ratio_points)):
            ...
            # Matplotlib gracefully breaks the line wherever y_ratio_points contains np.nan
    ...
    ax.yaxis.set_minor_locator(FixedLocator([1.5, 2.5, 3.5, 4.5]))
    ...
    ax.set_title(
        f"Relative Data Overhead Across Base Probability for Target Power: {int(TARGET_POWER_LEVEL*100)}\\%\n"
        f"($K={n}$, $M={m}$, $\\Delta p={p_diff}$ with Selection {selection_display_name} at $T=2000$)", 
    ...
    plot_name = "data_loss_ratio_over_p"
